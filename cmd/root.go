		...
		for _, projectPath := range args {
			err := filepath.Walk(
				...
				func(path string, info fs.FileInfo, err error) error {
					...
					projectBytes, err = readFileIntoSlice(path, projectBytes)
					if err != nil {
						return nil
					}
...
func readFileIntoSlice(name string, data []byte) ([]byte, error) {
	f, err := os.Open(name)
	if err != nil {
		return nil, err
	}
	...
	if size -= cap(data); size > 0 {
		data = append(data[:cap(data)], make([]byte, size)...)
	}
